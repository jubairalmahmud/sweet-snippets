...
return new class extends Migration {
    public function up(): void
    {
        Schema::create('frame_catalog', function (Blueprint $table) {
            $table->id();
            $table->string('code', 64)->unique();          // e.g. "egol", "king", "queen"
            $table->string('name', 120);
            $table->string('image_url', 500);              // stored asset URL or filename
            $table->unsignedInteger('price_coins')->default(0);
            $table->unsignedInteger('vip_level_required')->default(0);
            $table->enum('rarity', ['common','rare','epic','legendary'])->default('common');
            $table->unsignedInteger('duration_days')->default(30); // 0 = permanent
        ...
        Schema::create('user_frames', function (Blueprint $table) {
            ...
            $table->foreignId('frame_id')->constrained('frame_catalog')->cascadeOnDelete();
